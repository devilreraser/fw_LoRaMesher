...
void DebugTaskInit() {
    if (counterMutex == NULL && !initFailed) {
        ...
        if (counterMutex == NULL) {
            ESP_LOGE(TAG, "Failed to create counter mutex!");
            initFailed = true;
        }
    }
}
...
void DebugTaskCounter(const char* name) {
    ...
    if (counterMutex != NULL) {
        ...
        ESP_LOGE(TAG, "TaskLoopCounters array is full!");
        xSemaphoreGive(counterMutex);
    }
}
...
void DebugTaskResetStats() {
    ...
    if (counterMutex != NULL) {
        ...
        ESP_LOGI(TAG, "Task counters reset.");
        xSemaphoreGive(counterMutex);
    }
}
